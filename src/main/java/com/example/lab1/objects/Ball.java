...
public class Ball extends Circle {
	...
	public boolean handleCollision(final Bounds bounds) {
		...
		if (collisionDetected) {
			...
			final boolean isMinY = Utilities.areEqual(closestY, minY, epsilon);
			final boolean isMaxY = Utilities.areEqual(closestY, maxY, epsilon);
			final boolean isMinX = Utilities.areEqual(closestX, minX, epsilon);
			final boolean isMaxX = Utilities.areEqual(closestX, maxX, epsilon);
	...
	public void initializeFalling(final EventHandler<ActionEvent> onFinishHandler) {
	...
	private static enum State {
		MOVING,
		FALLING;
		private static Ball.State[] $values() {
			return new Ball.State[]{MOVING, FALLING};
		}
	}
}
